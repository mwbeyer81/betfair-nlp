# AGENTS.md archive — July 2026 (through the `auth-hardening` session)

This is the archived tail of `AGENTS.md`, moved out on 2026-07-25 to keep
the live file readable. These entries are historical record only — nobody
needs to "read this before touching X" the way the live file's warnings
apply. If you're chasing the history of a specific fix (e.g. the
index-backed-sort fix for `industry-sp-dao.ts`, or the raceCap/anonymous-
access design), it's here. For current in-progress work and active
worktrees, see `AGENTS.md`.

Entries below are unedited, moved verbatim from `AGENTS.md`.
