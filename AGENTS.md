...
| `~/betfair-nlp-live-filter-performance-drilldown` | `feat/live-filter-performance-drilldown` | Follow-up to the now-merged `feat/live-filter-performance` (see row above/history): user asked for the Live Performance section's meeting rows to be tappable, drilling into per-race/runner breakdown the same way `IspRacesScreen.tsx`'s Races view already does. Requires changing `saved_filter_set_live_results` from one doc per (filter set, meeting, day) to one doc per (filter set, race) — a real schema change to a collection with a small amount of just-captured real production data (14 docs from today's first live run); judged low-stakes (created minutes into this same feature's own testing, not yet depended on) and proceeding without asking, will simply repopulate correctly on the next cron run. **Touches `industry-sp-dao.ts` again** (still flagged hot). | in progress |
