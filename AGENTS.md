...
|---|---|---|---|
| `~/betfair-nlp-kaggle-racingapi-fields` | `kaggle-racingapi-fields` | Two related pieces of work. (1) **Research**, user-asked: compare every field the Kaggle results CSV and The Racing API each provide, both directions, at the level of *actual values* rather than field names — written up in `README-kaggle-vs-racingapi-fields.md`. No RacingAPI credentials exist in any checkout on this VM (`config/local.json` absent everywhere), so the API side was measured from Atlas after mapping (`daily_racecards` 573 races from `/racecards/free`; `industry_starting_prices` `raceDate >= 2026-07-01`, 529 races/4,221 runners from `/results/today`) plus the public OpenAPI spec for definitions; the CSV side from `mini-update.csv` (3,653 rows). The two corpora are different weeks so no runner overlaps — comparisons are distributional, or name-for-name across the ~700 horses/~300 trainers/~215 jockeys/~440 damsires that recur in both. Headline: 36 of the CSV's 37 columns have a same-named counterpart on `/results/today` (only `ran` doesn't), but `/racecards` is a *different shape* and `horse`/`sex`/`jockey`/`comment`/`prize`/`class` mean different things there. `comment` alone means four things (CSV in-running commentary with market moves; `/results` post-race analyst note; `/racecards` **pre-race** preview; race-level `comments` = stewards' notes) — measured consequence: the same `comment-lexicon.ts` categories fire at 53.2%/8.6% (`weakened`/`greenness`) on CSV text vs 25.5%/0.4% on API text, with `hung left`(82)/`green`(27)/`awkward start`(25) never appearing in API text and `dropped away`(489)/`faded`(260)/`boxed in`(45) never in CSV text, so `horseAvgExcuseScore` and friends are **not comparable across source eras**. Also found: prod's CSV-era runners have `rpr`/`ts` ~90% but **no `comment` field at all** (importer only started mapping it 2026-07-25, `92e38b1`, and the full import hasn't been re-run since), while API-era runners are the exact mirror — `comment` 84%, `rpr`/`ts` 0% (removed by the API in June 2026). No runner in the collection currently has both. (2) **Feature**, user-asked: a first-ever **admin permission** (`UserDocument.isAdmin`, granted only by `yarn grant:admin <email>` — deliberately unreachable from signup/Google/phone/any HTTP route), `AuthService.isAdmin` reading the flag from the DB on every call rather than from the JWT (so a revoke takes effect immediately), a `requireAdmin` route guard that answers 401/403/503 distinctly, `GET /api/admin/data-sources` behind it, and an admin-only `/admin/data-sources` screen rendering the comparison. The screen is gated server-side: the burger-menu item is hidden for non-admins, but a non-admin who types the URL gets an "Admins only" state driven by the endpoint's real 403, not a client-side decision. **Admin granted on production to `matthewbeyer@hotmail.com`** (`_id 6a5cd47b5b6d9dd065588d14`); `yarn grant:admin --list` confirms it is the only admin. | **in progress — committed on the branch, NOT merged and NOT deployed.** Verified locally: root `tsc --noEmit` and `client/yarn build` both clean. Supertest `app.test.ts` 271 passed/7 pre-existing skips (9 new cases incl. 401/403/200 on the admin route and `isAdmin` on `/api/auth/me`). New `user-dao-admin.integration.test.ts` 7/7 against real local Mongo; new `auth-service-admin.test.ts` 5/5. Storybook: new `DataSourceComparisonScreen.stories.tsx` 10/10; full suite 546/553 — the 7 failures are **pre-existing and reproduced identically on an unmodified `develop` checkout** (same 5 suites: EventsScreen, SavedResultsListScreen, AllRunnersScreen, RunnerDetailScreen, IndustrySpScreen). `yarn test:msw` 317/317 (8 new in `admin-data-sources.spec.ts`). Full `npx jest` 842 passed/42 failed — those 42 also reproduce identically on an untouched checkout (betfair-service, openai-integration, and the price-update/market-definition/bet-order integration suites). New live spec `client/tests-live/admin-data-sources-live.spec.ts` run against prod **before** deploying: 2 passed, 3 failed, 2 skipped — exactly the expected pre-deploy shape, since the deployed Lambda has no `/api/admin/data-sources` and the deployed bundle has no `/admin/data-sources` route. Its admin half needs `ADMIN_EMAIL`/`ADMIN_PASSWORD` env vars and skips loudly without them. **Next step is a decision for the user, not an agent:** merge to `develop` and run `apps/lambda/build.sh` + `apps/web/deploy.sh`, after which the live spec should go green. |
