...
| `~/betfair-nlp-daily-race-model-factors` | `daily-race-model-factors` | User asked whether the Model % pill can "explain itself" — follow-up to the three rows above. Adds a plain-language "top factors" list (top 3, e.g. "Strong recent form", "In-form trainer") to the existing Model % tooltip on `DailyRaceScreen.tsx`, computed via XGBoost's native `pred_contribs=True` (exact SHAP values, no new Python dependency) in `apps/ml-api/handler.py`, restricted to the 22 `NUM_COLS` (skips the 8 `CAT_COLS` identity columns — no clean "helped/hurt" phrasing for those). Threads a new `topFactors`/`modelTopFactors` field through `prediction-api-client.ts` → `daily-race-service.ts` → `daily-race-dao.ts` → `chatApi.ts` → the tooltip. Scoped to the live Daily Race pipeline only — historical ISP screens (`RunnerDetailScreen`, `IndustryRaceScreen`, `IndustrySpScreen`, `IndustryMeetingScreen`) use a separate write path and are out of scope. Plan: `/home/ubuntu/.claude/plans/atomic-purring-puffin.md`. | **implemented and verified, not yet merged/deployed** — prototyped `pred_contribs` against the CI-fixture model first (contributions reconstruct the exact margin score, diff ~2e-7); `apps/ml-api/test_handler.py` 13/13 pass; backend `tsc --noEmit` clean, Supertest `app.test.ts` daily-races block 14/14 pass; `yarn build` clean; Storybook `DailyRaceScreen.stories.tsx` 16/16 pass (new `ModelFactorsRendered`); MSW suite 194/194 pass (new "Model % tooltip shows plain-language top factors" test) — 6 unrelated pre-existing story-test failures elsewhere (IndustrySpScreen/AllRunnersScreen/IspRacesScreen/SavedResultsListScreen/EventsScreen/RunnerDetailScreen) confirmed untouched by this branch's diff. Reverted an unrelated, unexplained root `yarn.lock` rewrite that `npm install` produced in this fresh worktree before committing anything. Awaiting user go-ahead to commit/merge/deploy. |
