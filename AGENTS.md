...
| `~/betfair-nlp-header-overlap-fix` | `header-overlap-fix` | Fix the Appbar header buttons (incl. the new "Model Performance" button) overlapping the "BackBet" title on narrow/mobile viewports in production; add persistent narrow-viewport MSW/Playwright tests | committed (`c7b10d5`), merged `origin/develop` in cleanly (no conflicts), about to merge to local `develop` and deploy — see dated entry below |
