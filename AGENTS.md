...
**Verified**: root `tsc`/`yarn build` clean, `cd client && yarn build` clean. Merged, deployed (Lambda — `apps/lambda/build.sh`), health-checked (`/api/stats` → 401). Re-ran the exact same prod-repro spec after deploying the fix: passed — Bets tab loaded real data with no error. Whether this specific run would have hit an affected container either way is inherently non-deterministic (that's the nature of the bug), but the underlying mechanism (a stuck-forever container) is now structurally impossible — the mocked jest tests are the deterministic proof of that, this spec is the live confirmation the ordinary path still works end to end.
