...
| `~/betfair-nlp-daily-race-model-factors` | `daily-race-model-factors` | User asked whether the Model % pill can "explain itself" — follow-up to the three rows above. Adds a plain-language "top factors" list (top 3, e.g. "Strong recent form", "In-form trainer") to the existing Model % tooltip on `DailyRaceScreen.tsx`, computed via XGBoost's native `pred_contribs=True` (exact SHAP values, no new Python dependency) in `apps/ml-api/handler.py`, restricted to the 22 `NUM_COLS` (skips the 8 `CAT_COLS` identity columns — no clean "helped/hurt" phrasing for those). Threads a new `topFactors`/`modelTopFactors` field through `prediction-api-client.ts` → `daily-race-service.ts` → `daily-race-dao.ts` → `chatApi.ts` → the tooltip. Scoped to the live Daily Race pipeline only — historical ISP screens (`RunnerDetailScreen`, `IndustryRaceScreen`, `IndustrySpScreen`, `IndustryMeetingScreen`) use a separate write path and are out of scope. Plan: `/home/ubuntu/.claude/plans/atomic-purring-puffin.md`. | **merged to `develop`, deploy in progress** — prototyped `pred_contribs` against the CI-fixture model first (contributions reconstruct the exact margin score, diff ~2e-7); `apps/ml-api/test_handler.py` 13/13 pass; backend `tsc --noEmit` clean, Supertest `app.test.ts` daily-races block 14/14 pass; `yarn build` clean; Storybook `DailyRaceScreen.stories.tsx` 16/16 pass. **Merging `origin/develop` (which had picked up `fix/daily-race-pill-wrap`'s `pillGroup` change concurrently) surfaced a real regression**: the MSW fixture gave `hrs_1` — the exact runner the shared drill-down test clicks by its whole-row testID — Model/Fair-odds badges too, and `pillGroup`'s new wrapping shifted that row's bounding-box center onto a nested pill (whose `onPress` calls `stopPropagation`), silently breaking real-browser click navigation 3/3 times on repeat (Storybook's `NavigationTriggered` story still passed, since Testing Library's `userEvent.click` targets the DOM node directly rather than doing real coordinate hit-testing — confirms this is Playwright-real-click-specific, i.e. a real click-target risk on any runner row with all three badges, not just a test artifact). Fixed by moving the model/factors fixture data off `hrs_1` onto `hrs_2` instead, leaving the shared navigation test's runner unchanged. Full MSW suite confirmed clean at 194/194 after the fix (two earlier full-suite attempts hit unrelated infra flakiness — OOM/`ERR_CONNECTION_REFUSED` server crashes — from 2-3 concurrent Claude sessions competing for memory on this VM, not real regressions; the daily-races-specific spec was re-verified clean across 3 separate runs throughout). **Heads-up for anyone touching `DailyRaceScreen.tsx`'s row tap target**: a runner with form+model+fair-odds badges simultaneously can have its row's tap-navigate silently swallowed if a real tap lands on a pill — worth a real hit-area redesign at some point, out of scope here. Reverted an unrelated, unexplained root `yarn.lock` rewrite that `npm install` produced in this fresh worktree before committing anything. |
