...
const TestComponent = ({ onButtonPress, onInputChange, placeholder = "Enter text..." }) => {
...
export default {
  ...
  parameters: {
    docs: {
      description: {
        component: 'This example demonstrates comprehensive logging in interaction tests.',
      },
    },
  },
  argTypes: {
...
BasicInteraction.play = async ({ canvasElement, args }) => {
  await runLoggedTest('Examples/LoggingExample', 'BasicInteraction', canvasElement, args, async (context) => {
    const canvas = loggedWithin(canvasElement, context);
    
    // Log test setup
    testLogger.info(context.storyName, context.testName, 'Setting up basic interaction test');
    ...
    loggedExpect(args.onButtonPress, context).toHaveBeenCalledWith('Hello World!');
    
    // Log state changes
    testLogger.stateChange(context.storyName, context.testName, 'TestComponent', 'input filled and submitted', {
...
ComplexInteraction.play = async ({ canvasElement, args }) => {
  await runLoggedTest('Examples/LoggingExample', 'ComplexInteraction', canvasElement, args, async (context) => {
    const canvas = loggedWithin(canvasElement, context);
    
    testLogger.info(context.storyName, context.testName, 'Starting complex interaction test');
    ...
    testLogger.info(context.storyName, context.testName, 'Typed first part of message');
    
    // Test clearing and retyping
    await loggedUserEvent.type(input, 'Second', context);
    testLogger.info(context.storyName, context.testName, 'Typed second part of message');
    ...
    loggedExpect(args.onButtonPress, context).toHaveBeenCalledWith('FirstSecond');
    
    // Log comprehensive state
    testLogger.stateChange(context.storyName, context.testName, 'TestComponent', 'complex interaction completed', {
...
ErrorHandling.play = async ({ canvasElement, args }) => {
  await runLoggedTest('Examples/LoggingExample', 'ErrorHandling', canvasElement, args, async (context) => {
    const canvas = loggedWithin(canvasElement, context);
    
    testLogger.info(context.storyName, context.testName, 'Testing error handling scenarios');
    ...
    await loggedUserEvent.click(button, context);
    loggedExpect(args.onButtonPress, context).toHaveBeenCalledWith('Special chars: !@#$%^&*()');
    
    testLogger.success(context.storyName, context.testName, 'Error handling test completed successfully');
  });
};
